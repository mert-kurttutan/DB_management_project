...
RecordId Page::insertRecord(const std::string& record_data) {
  ...
  // std::cout << "Mert -getting available slot\n";
  //getAvailableSlot();
  // std::cout << "Mert - using getavaialble \n";
  try
  {

    //std::cout << "Mert - see if here !!!!"; 
  ...
  PageSlot* slot_my = getSlot((const SlotId)(slot_number));
...
SlotId Page::getAvailableSlot() {
  ...
  //std::cout << "Mert - defined slot_number \n";
  if (header_.num_free_slots > 0) {
  ...
  static_cast<SlotId>(slot_number);
...
void Page::insertRecordInSlot(const SlotId slot_number,
  ...
  PageSlot* slot2 = getSlot(slot_number);
